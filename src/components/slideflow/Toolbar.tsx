...
import { MousePointer, Pen, Highlighter, Eraser, Undo, Trash2, MousePointer2, Type, Square, Circle, ArrowUpRight } from 'lucide-react';
...
const tools: { name: Tool, icon: React.ElementType, label: string }[] = [
    ...
    { name: 'text', icon: Type, label: 'Text Tool' },
];

const shapeTools: { name: Tool, icon: React.ElementType, label: string }[] = [
  { name: 'rectangle', icon: Square, label: 'Rectangle Tool' },
  { name: 'circle', icon: Circle, label: 'Circle Tool' },
  { name: 'arrow', icon: ArrowUpRight, label: 'Arrow Tool' },
];
...
export default function Toolbar() {
    ...
    return (
        <div className="flex items-center gap-1 p-1 bg-background rounded-lg shadow-lg border">
            ...
            {(activeTool === 'pen' || activeTool === 'highlighter' || activeTool === 'text' || isShapeToolActive) && (
              <>
                <Popover>
                    <PopoverTrigger asChild>
            ...
            <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive hover:bg-destructive/10" aria-label="Clear all annotations on this slide" title="Clear slide" onClick={() => clearAnnotations(currentPage)}>
